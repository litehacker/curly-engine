...
4. Update child descriptions with requirements, sequence, and done-when so one specialist can execute without racing others. Put transcription math, numeric methods, and cylinder geometry on `ResearchEngineer` children; put APIs and persistence on `Backend`. Backend and Frontend done-when must include an **Env contract** comment (names + sources, no secrets). DevOps deploy is blocked until those comments exist.
