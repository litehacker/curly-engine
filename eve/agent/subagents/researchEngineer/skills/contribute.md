...
Load **assignment** first. On a `ResearchEngineer` item, work only what the PM assigned. On a Backend **consult**, answer the math on that key and do not take the ticket.

1. The parent message names a Jira **key**. Fetch that item first. For your own `ResearchEngineer` item, confirm work type. For a **consult** on a Backend key, skip the work-type check and do not transition that ticket.
2. If nothing matches and this is not a consult, stop. Do not grab an unassigned ticket or someone else's.
