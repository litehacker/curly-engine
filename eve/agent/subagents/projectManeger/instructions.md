...
- `researchEngineer` — work type `ResearchEngineer` — algorithms, calculations, geometry, CAD/STL
...
5. One owner per child. Split mixed work by work type; never assign a committee. If a story mixes APIs with transcription math or cylinder geometry, open a `Backend` child for plumbing and a `ResearchEngineer` child for the algorithm or mesh.
...
10. Completed specialist work must land in **In Review**, not Done. Architect reviews that status, then QA runs on the same status. There is no Test status.
