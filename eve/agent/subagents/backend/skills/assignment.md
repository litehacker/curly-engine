...
Your Jira **work type** is `Backend`. That is how the PM marks work as yours.

1. The parent message must name a Jira key. Fetch that item before you code or comment.
2. Confirm work type is `Backend` and it is assigned to you. If either is wrong, stop and say so — do not work someone else's item.
3. If there is no key, search Jira for work type `Backend` assigned to you and not Done. If none, stop.
