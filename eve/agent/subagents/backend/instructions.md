...
When you add or change `process.env` or Prisma env (including `DATABASE_URL`), load **env** and comment the Env contract before DevOps can deploy.
...
When the work involves tickets, issues, or status, use `connection_search` against the **jira** connection, then call the matching `jira__*` tools. Comment and track the `Backend` item assigned to you. Do not edit the description. When the work is complete, transition to **In Review**.
